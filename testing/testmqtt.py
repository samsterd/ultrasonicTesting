...
mqtt_client = mqtt_connect("tester")
...
experimentParams = {
    ...
    'experimentFolder': '../TestData/dbtest06-19/',                     # Name of folder to dump data
    'experimentName' : 'TestDB10',                  # File name for single scan and repeat pulse experiment. Will be appended with .json or .sqlite3
...
waveData = open("./WaveDataRawData", 'r')
waveDataKeys = open("./WaveDataRawKeys", 'r')
data = waveData.readline()
keys = waveDataKeys.readline().rstrip('\n')

print(data)
print(keys)
experimentParams['fileName'] = os.path.join(experimentParams['experimentFolder'], experimentParams['experimentName'])
scanFileName = os.path.join(experimentParams['experimentFolder'], experimentParams['experimentName'])
print(experimentParams['fileName'])
mqtt_handle_params(mqtt_client, experimentParams)
time.sleep(5)
mqtt_quick_pub(mqtt_client, mqtt_publish_properties, data, key=keys, file=scanFileName, topic="/pulser/WaveData/")
