...
experimentParams = {
    ...
    'axis': 'Z',                                     # Axis to move scanner
    'distance': -5,                                  # Distance in mm to move scanner
    ...
    'transducerFrequency' : 0.5,                    # Central frequency of the ultrasonic transducer, in MHz. Current options are 2.25 or 50
    'pulserType' : 'tone burst',                       # Type of pulser. 'standard' is the single wave CompactPulser. 'tone burst' uses the USBUT350 tone burst pulser
    'measureTime' : 480,                               # Approx measurement time, in us. Note this can be changed by the picoscope time interval based on samples
                                                     #      Changes to the measureTime will be printed in the console when the script is run
    'measureDelay' : 50,                           # Approx delay after trigger to start measuring, in us
    ...
    'samples': 20000,                                  # Number of data points per wave
    'halfCycles' : 2,                               # Tone burst pulser only. Number of half-cycles in a tone burst pulse. Minimum 1, maximum 32
    ...
    'primaryAxisRange' : 30,                         # Distance in mm to scan on the primary axis
    'primaryAxisStep' : 0.5,                        # Distance in mm between each scan on the primary axis
    'secondaryAxisRange' : 50,                       # Distance in mm to scan on the secondary axis
    'secondaryAxisStep' : -0.5,                     # Distance in mm between each scan on the secondary axis
    ...
    'pulseInterval' : 1800,                            # For repeat pulse: Minimum time between pulse collection, in seconds
    'experimentTime' : 86400,                           # For repeat pulse: Time to collect data, in seconds
    ...
    'scannerPort' : 'COM5',                         # Ender port name
