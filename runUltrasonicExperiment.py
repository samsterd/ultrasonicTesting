...
#
# 3) run this script. On PyCharm, simply press Shift + F10 or click the green play button at the top right
...
experimentParams = {
    ...
    'axis': 'Z',                                     # Axis to move ender
    'distance':-40,                                  # Distance in mm to move ender
    ...
    'pulserType' : 'standard',                       # Type of pulser. 'standard' is the single wave CompactPulser. 'tone burst' uses the USBUT350 tone burst pulser
                                                     # NOTE: Drivers for tone burst pulser are not installed on linux computer
    ...
    'experimentName' : 'autorange_test_8',                 # File name for single scan and repeat pulse experiment. Will be appended with .json
    'experimentBaseName' : 'sa_1_3a_2MLiDFOB_wetting',   # Base filename for multi scan experiment, which will have the scan # appended to it
    'saveFormat' : 'sqlite',                        # Format to save data. Options are sqlite or json. Sqlite is recommended
    'pickleData' : False,                            # An alternate saving format which saves the data as a python dict. Adds ~10s overhead per scan in multi scan, no overhead in single scan or repeat pulse
    ...
    'scanInterval' : 1800,                          # For multi scans: Minimum time between start of scans, in seconds.
    'numberOfScans' : 240,                           # For multi scans: Number of times to run the scan
    'pulseInterval' : 1,                           # For repeat pulse: Minimum time between pulse collection, in seconds
    'experimentTime' : 10,                        # For repeat pulse: Time to collect data, in seconds
